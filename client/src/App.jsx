    ...
    <main>
      <section id="home" className="hero container">
        <div className="hero-copy">
          ...
          <h1>Prajakta<br /><span>Khochage.</span></h1>
      ...
      <Section id="about" number="01" eyebrow="Introduction" title="A curious builder with a practical mindset.">
        <div className="about-grid">
          ...
          <div className="about-side"><div className="fact-card glass-card"><span className="fact-icon"><FiMapPin /></span><small>Based in</small><b>Kolhapur, Maharashtra</b></div><div className="fact-card glass-card"><span className="fact-icon"><FiBookOpen /></span><small>Currently pursuing</small><b>B.E. CSE · 2024–2027</b></div><div className="fact-card glass-card"><span className="fact-icon"><FiGlobe /></span><small>Languages</small><b>English · Hindi · Marathi · Japanese</b></div></div>
        </div>
      </Section>
      ...
      <Section id="projects" number="03" eyebrow="Featured work" title="Things I've built, explored and shipped.">
        <div className="project-grid">{projects.map((p,i) => <Reveal className={`project-card glass-card ${i === 0 ? 'featured' : ''}`} key={p.title}><div className="project-glow" /><div className="project-top"><span>0{i+1}</span><small>{p.category}</small></div><div className="project-icon">{i===0?<FiBriefcase/>:i===1?<FiCpu/>:i===2?<FiBarChart2/>:<FiBookOpen/>}</div><h3>{p.title}</h3><p>{p.description}</p><div className="project-highlight"><FiCheckCircle /> {p.highlight}</div><div className="chips">{p.tech.map(t => <span key={t}>{t}</span>)}</div><div className="project-links"><a href={p.github} target="_blank" rel="noreferrer"><FiGithub /> Source <FiArrowUpRight /></a>{p.demo !== '#' && <a href={p.demo} target="_blank" rel="noreferrer">Live demo <FiExternalLink /></a>}</div></Reveal>)}</div>
      </Section>

      <Section id="skills" number="04" eyebrow="Tech arsenal" title="Tools I use to turn ideas into products.">
        <div className="skill-grid">{skills.map((s) => { const Icon = iconMap[s.icon]; return <Reveal className="skill-card glass-card" key={s.group}><div className="skill-head"><span className="skill-icon"><Icon /></span><h3>{s.group}</h3></div><div className="chips">{s.items.map(x => <span key={x}>{x}</span>)}</div></Reveal> })}</div>
      </Section>
      ...
      <Section id="education" number="07" eyebrow="Academic background" title="The foundation behind the work.">
        <div className="education-grid">{education.map((e,i)=><Reveal className="education-card glass-card" key={e.degree}><div className="edu-number">0{i+1}</div><span>{e.period}</span><h3>{e.degree}</h3><h4>{e.org}</h4><p>{e.detail}</p></Reveal>)}</div>
      </Section>
